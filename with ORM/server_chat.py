...
class Server(Server_config):
    ...
    def showing_all_msg(self, client, chatRoomNumber_id):
        messages = model_methods.all_messages(chatRoomNumber_id[0])
    ...
    def messages_to_str_list(messages):
        ...
        for unit in messages:
            ...
            unit_list.append(unit.message)
            unit_list.append(unit.sender_id)
            unit_list.append(unit.receivers_id)
            unit_list.append(unit.chatRoomNumber_id)
            unit_list.append(unit.created_at)

            message = unit_list[1]
            sender_id = unit_list[2]
            sender = model_methods.getting_username_by_id(sender_id)
            print(f'sender is {sender}')
            receiver_result = ""
            created_at = unit_list[len(unit_list)-1]


            count = 0
            for ele in unit_list:
                if count == 3:
